...
def process_document(filename, file_bytes):
    ...
    Streamlit caches the result so the same document
    does not need to be embedded again.
    """
...
def safe_call(function, **kwargs):
    """
    Call a gdown function while supporting older and newer
    gdown versions by dropping unsupported keyword arguments.
    """
...
def download_drive_file(url, output_dir):
    ...
    if not output_path and file_id:
        output_path = safe_call(
            gdown.download,
            id=file_id,
            output=str(output_dir) + os.sep,
...
def load_from_drive(url):
    ...
    # Single file
        ...
        downloaded = download_drive_file(url, temp_dir)
        files = [downloaded] if downloaded else []
...
def file_fingerprint(filename, file_bytes):
    """Create a unique document fingerprint."""
...
# GLOBAL STYLING
#
# NOTE: every st.markdown HTML block below starts at
# column 0. Indenting HTML by 4+ spaces makes Streamlit's
# Markdown parser treat it as a code block and print the
# raw tags on screen instead of rendering them.
# =========================================================
...
[data-testid="stFileUploader"] section { background: var(--bg-card) !important; }
[data-testid="stFileUploaderDropzone"] { background: var(--bg-card) !important; }
...
<div class="hero-sub">Upload files or pull them from Google Drive, then ask questions answered with hybrid semantic + keyword search over your own documents.</div>
</div>
""", unsafe_allow_html=True)
...
<span class="section-badge">1</span> Local documents
</div>
""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="section-label">
<span class="section-badge">2</span> Google Drive
</div>
""", unsafe_allow_html=True)
...
if st.session_state.files:
    ...
    st.markdown("""
<div class="section-label">
<span class="section-badge">📊</span> Document overview
</div>
""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="section-label">
<span class="section-badge">3</span> Ask a question
</div>
""", unsafe_allow_html=True)
        ...
        if not question.strip():
            ...
            for number, result in enumerate(results, start=1):
                ...
                with st.expander(
                    ...
                    st.markdown(
                        '<div class="score-row">'
                        f'<span class="score-pill hybrid">Hybrid {result["hybrid_score"]:.3f}</span>'
                        f'<span class="score-pill">Semantic {result["semantic_score"]:.3f}</span>'
                        f'<span class="score-pill">Keyword {result["keyword_score"]:.3f}</span>'
                        '</div>',
                        unsafe_allow_html=True
                    )
